...
export default {
  ...
  methods:{
      ...
      /**@augments page 页数   不传  默认1  传就是当前页 */
        initPage(page){
             this.pageObj.filter = "hm_archives.typeid=1 and hm_archives.member="+JSON.parse(ykp.getLocalStorage('userInfo')).mid;
            this.pageObj.order = "hm_archives.pubdate desc";
            this.pageObj.auth = 1;
            // this.pageObj.filter = "member_stow.mid="+JSON.parse(ykp.getLocalStorage('userInfo')).mid + " and member_stow.type = 1";
